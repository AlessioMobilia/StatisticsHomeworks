...
namespace Cantelli
{
    ...
    public class Charts
    {
        ...
        public void drawHorizzontalLine(double y, string label, Graphics g)
        {
            ...
            g.DrawLine(axisPen, XRefactor(0), YRefactor(y), XRefactor(maxX), YRefactor(y));
            SizeF size = g.MeasureString(label, drawFont);
            g.DrawString(label, drawFont, fontBrush, XRefactor(0) - size.Width - 5, YRefactor(y) - size.Height / 2);
        ...
        public void drawXLabel(double step, Graphics g)
        {
            

            for (double x=minX; x<=maxX; x += step)
            {
                SizeF size = g.MeasureString(x.ToString(), drawFont);
                g.DrawLine(RugPen, XRefactor(x), YRefactor(minY), XRefactor(x), YRefactor(minY) - 10);
                g.DrawString(x.ToString(), drawFont, fontBrush, XRefactor(x), YRefactor(minY) + size.Height);
            }
        ...
        public void drawYLabel(double step, Graphics g)
        {


            for (double y = minY+step; y < maxY; y += step)
            {
                SizeF size = g.MeasureString(y.ToString(), drawFont);
                g.DrawLine(RugPen, XRefactor(minX), YRefactor(y), XRefactor(minX), YRefactor(y) - 10);
                g.DrawString(y.ToString(), drawFont, fontBrush, XRefactor(minX) - size.Width - 10, YRefactor(y));
            }
        ...
        public void drawVerticalLine(double x, Graphics g)
        {
            ...
            g.DrawLine(axisPen, XRefactor(x), YRefactor(0), XRefactor(x), XRefactor(maxY));
        ...
        public void DrawHistogram(List<Interval> intervals, Graphics g)
        {
            ...
            foreach (Interval inter in intervals)
            {
                ...
                int height = (int)inter.value*viewPort.Width/90;
                int x;
                if (inter.start > minX)
        ...
        public void DrawLadder(Dictionary<double, double> realPoints, Graphics g)
        {
            List<Point> points = new List<Point>();


            double lastY = -1;
            foreach (KeyValuePair<double,double> kv in realPoints)
            {
                if(lastY!=-1)
                    points.Add(new Point(XRefactor(kv.Key), YRefactor(lastY)));
                points.Add(new Point(XRefactor(kv.Key), YRefactor(kv.Value)));
                lastY = kv.Value;
