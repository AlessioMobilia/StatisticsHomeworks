...
namespace ReadCSV2
{
    ...
    public class Charts
    {
        ...
        public Pen linePen { set; get; } = new Pen(Brushes.DarkRed, 1.5f);
        ...
        public void drawHorizzontalLine(double y, string label, Graphics g)
        {
            if(y>=minY && y <= maxY)
            {
                g.DrawLine(linePen, XRefactor(minX), YRefactor(y), XRefactor(maxX), YRefactor(y));
        ...
        {
            if (x >= minX && x <= maxX)
                g.DrawLine(linePen, XRefactor(x), YRefactor(minY), XRefactor(x), YRefactor(maxY));

        }



        public void DrawTable(List<double[]> table,List<Interval> distX, List<Interval> distY, Graphics g)
        {
            ...
            foreach (double[] row in table)
            {
                ...
                {
                    g.DrawString(element.ToString("0.##"), tableFont, fontBrush, x, y);
                    x += width;
                }
        ...
        public void DrawHistogram(List<Interval> intervals, int countTotal ,Graphics g)
        {
